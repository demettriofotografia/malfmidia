...
const InstagramPostFrame: React.FC<{ 
  ...
  // Sincroniza o play/pause e o som baseado no estado global da seção
  useEffect(() => {
    if (videoRef.current) {
      if (isCurrentActive) {
  ...
  const handleTogglePlay = () => {
    if (isCurrentActive) {
      setActiveId(null); // Se já está tocando, pausa
    } else {
      setActiveId(item.id); // Toca este e o useEffect pausa os outros
    }
  };
  ...
  return (
    <div className="relative w-full bg-white/5 backdrop-blur-md rounded-xl border border-white/10 p-3 shadow-2xl transition-all duration-500 hover:border-[#ff4d00]/30 group">
      {/* Top Header - Nome dinâmico baseado no arquivo */}
      ...
      >
        <video 
          ref={videoRef}
          src={item.videoUrl} 
          // Se estiver ativo: colorido e opacidade total. Se não: P&B e opacidade reduzida.
          className={`w-full h-full object-cover transition-all duration-700 ${
            isCurrentActive ? 'grayscale-0 opacity-100' : 'grayscale opacity-60'
          }`}
          loop 
          playsInline
          muted // Começa mudo por padrão do navegador
        />
        
        {/* Ícone de Play - Só aparece se o vídeo NÃO estiver ativo */}
        {!isCurrentActive && (
        ...
        <div className="absolute bottom-4 left-4 pointer-events-none">
          <span className="block text-white font-black text-[10px] uppercase tracking-tighter mb-0.5">PRODUÇÃO</span>
          <span className="block text-[#ff4d00] font-black text-[8px] uppercase tracking-widest">MALF MIDIA</span>
        </div>
      </div>
    ...
    <div className="relative bg-transparent overflow-hidden min-h-screen py-24">
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <span className="text-[#ff4d00] text-xs uppercase tracking-widest mb-4 block font-bold">o processo invisível</span>
        ...
        <div className="flex overflow-x-auto lg:grid lg:grid-cols-4 gap-6 no-scrollbar snap-x snap-mandatory">
          {videoItems.map((item) => (
